...
def configureBoardPosition(img, areaSize = 1000):
    ...
    if squares is not None:
        ...
        ## --- Found possible board, trying to match with preset
    ...
    ## --- "Board not found" handling
    print("FAIL")
    if (img.size > areaSize+1000):
        ## --- Trying with larger squares
        return configureBoardPosition(img, areaSize+1000)
    else:
        ## --- Max size reached, return not found
        return -1,-1,-1,-1
...
-->              ['P', '-', 'P']
-->              ['-', 'P', '-'] where 'P' can be any non-'-' character
-->              ['P', '-', 'P']
-->          - False if it isn't
"""
